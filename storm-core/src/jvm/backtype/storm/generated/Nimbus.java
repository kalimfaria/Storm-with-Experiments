...
public class Nimbus {
  ...
  public static class Client extends org.apache.thrift.TServiceClient implements Iface {
    ...
    {
      send_getTopology(id);
      StormTopology temp = recv_getTopology();
      File advanced_scheduling_log = new File("/var/nimbus/storm/advanced_scheduling_log.log");
      try {
        FileWriter fileWriter = new FileWriter(advanced_scheduling_log, true);
        BufferedWriter bufferWriter = new BufferedWriter(fileWriter);
        bufferWriter.append("\n *** Logging from Nimbus.java *** \n");
        Map <String, Bolt> bolts =  temp.get_bolts();
        Map <String, SpoutSpec> spouts =  temp.get_spouts();

        for ( Map.Entry<String, Bolt> boltsEntry : bolts.entrySet())
        {
          bufferWriter.append("\nBolt key: " + boltsEntry.getKey() + " parallelism hint:  " + boltsEntry.getValue().get_common().get_parallelism_hint());
        }

        for ( Map.Entry<String, SpoutSpec> spoutsEntry : spouts.entrySet())
        {
          bufferWriter.append("\n Spout key: " + spoutsEntry.getKey() + " parallelism hint:  " + spoutsEntry.getValue().get_common().get_parallelism_hint());
        }

        bufferWriter.append("\n *** Done Logging from Nimbus.java *** \n");
        bufferWriter.close();
        fileWriter.close();
