...
(ns backtype.storm.command.henge-rebalance
...
(defn -main [& args]
  (def i 0) ;; remove this later
  (def combined (join " " args))
  (prn combined)
  (def separated (split combined #"xyz"))
  (prn separated)
  (doseq [item separated]
    (def i (inc i)) ;; remove if error
    (def one_top (split item #" "))
    (prn i)
    (if (> i 1) (def one_top (subvec (split item #" ") 1)))
    (prn one_top)
    (let [[{wait :wait executor :executor num-workers :num-workers} [name] _]
          (cli one_top ["-w" "--wait" :default nil :parse-fn #(Integer/parseInt %)]
      ...
      (with-configured-nimbus-connection nimbus
        ...
        (prn "Topology " name " is rebalancing")
        )
